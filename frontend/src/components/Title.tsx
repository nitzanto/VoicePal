    ...
    await await axios
      ...
      .then((res) => {
        if (res.status == 200)
          setMessages(
            []
          ); // Sending a request to delete the data from .json, will create an empty list
        else
          console.error(
            "There was an error with the API request to backend (server side)"
          );
      })
      .catch((err) => {
  ...
  return (
    <div className="flex justify-between items-center w-full p-4 bg-gray-900 text-white font-bold shadow">
      <div className="flex items-center italic">
        {" "}
        <img
          src="https://i.imgur.com/hYkfzYT.png"
          className="w-13 h-13 mr-2 filter brightness-0 invert"
        />
        VoicePal
      </div>
      <button
